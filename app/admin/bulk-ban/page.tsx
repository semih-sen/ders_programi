...
async function getUsers(gradeFilter?: number) {
  const whereClauses: any[] = [{ isBanned: false }];
...
export default async function Page({
  ...
  function buildUrl(period?: number) {
    ...
    return `/admin/bulk-ban${qs ? `?${qs}` : ''}`;
  }

  return (
    <div className="p-6 space-y-4">
      <div className="flex flex-col gap-2">
        <h1 className="text-xl font-semibold text-white">Toplu Banlama</h1>
        <PeriodTabs activePeriod={gradeFilter} buildHref={buildUrl} />
        <p className="text-sm text-slate-400">
          Sadece banlı olmayan kullanıcılar listelenir. Seçili sekmeye göre filtrelenir, dönemsiz kullanıcılar her sekmede görünür.
        </p>
      </div>
      <Client users={users} />
    </div>

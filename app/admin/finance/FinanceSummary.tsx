  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Devreden Varlık */}
        <div className="bg-gradient-to-br from-slate-600/30 to-slate-700/30 backdrop-blur-sm rounded-xl border border-slate-600/50 p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-slate-300 font-semibold text-sm">Devreden Varlık</h3>
            <span className="text-2xl">📦</span>
          </div>
          <p className="text-3xl font-bold text-slate-200 mb-1">
            {formatCurrency(openingBalance)}
          </p>
          <p className="text-xs text-slate-400">Dönem başındaki kasa</p>
        </div>
        ...
        <div className={`bg-gradient-to-br ${
          ...
          <div className="flex items-center justify-between mb-2">
            <h3 className={`${currentBalance >= 0 ? 'text-green-300' : 'text-red-300'} font-semibold text-sm`}>
              Bugünkü Kasa
            </h3>
            <span className="text-2xl">{currentBalance >= 0 ? '💰' : '⚠️'}</span>
          </div>
          <p className={`text-3xl font-bold ${currentBalance >= 0 ? 'text-green-200' : 'text-red-200'} mb-1`}>
            {formatCurrency(currentBalance)}
          </p>
          <p className="text-xs text-slate-400">Şu anki anlık bakiye</p>
        </div>

        {/* Devredecek Varlık (Tahmini) */}
        <div className="bg-gradient-to-br from-blue-500/20 to-cyan-600/20 backdrop-blur-sm rounded-xl border border-blue-500/30 p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-blue-300 font-semibold text-sm">Devredecek Varlık</h3>
            <span className="text-2xl">🔮</span>
          </div>
          <p className="text-3xl font-bold text-blue-200 mb-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-500/20 to-emerald-600/20 backdrop-blur-sm rounded-xl border border-green-500/30 p-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-green-300 font-semibold">Dönem Gelirleri</h3>
            <span className="text-2xl">📈</span>
          </div>
          <p className="text-3xl font-bold text-green-200 mb-3">
        ...
        } backdrop-blur-sm rounded-xl border p-6 ring-2 ${
          netChange >= 0 ? 'ring-blue-400/30' : 'ring-rose-400/30'
        }`}>
          <div className="flex items-center justify-between mb-3">
            <h3 className={`${netChange >= 0 ? 'text-blue-300' : 'text-rose-300'} font-semibold`}>
              Dönem Net Farkı
            </h3>
            <span className="text-3xl">{netChange >= 0 ? '📊' : '📉'}</span>
          </div>
          <p className={`text-4xl font-bold ${netChange >= 0 ? 'text-blue-100' : 'text-rose-100'} mb-2`}>
        ...
        <div className="bg-gradient-to-br from-red-500/20 to-rose-600/20 backdrop-blur-sm rounded-xl border border-red-500/30 p-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-red-300 font-semibold">Dönem Giderleri</h3>
            <span className="text-2xl">📉</span>
          </div>
          <p className="text-3xl font-bold text-red-200 mb-3">

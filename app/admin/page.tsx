  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-slate-400">Sistem istatistikleri ve özet bilgiler</p>
      </div>
...
function StatCard({ title, value, icon, color }: { title: string; value: number; icon: string; color: string }) {
  ...
  return (
    <div className={`bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border ${c.border}`}>
      <div className={`w-12 h-12 ${c.bg} rounded-lg flex items-center justify-center text-2xl mb-4`}>{icon}</div>
...
function QuickLink({ href, title, description, color }: { href: string; title: string; description: string; color: string }) {
  return (
    <a href={href} className="flex items-center p-4 bg-slate-700/50 hover:bg-slate-700 rounded-lg transition-colors group">
      <div className={`w-12 h-12 ${color === 'blue' ? 'bg-blue-500/20' : 'bg-purple-500/20'} rounded-lg flex items-center justify-center mr-4`}>
        <svg className={`w-6 h-6 ${color === 'blue' ? 'text-blue-400' : 'text-purple-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          {color === 'blue' ? (
